...
class discordClient(discord.Client):
                ...
                while not self.message_queue.empty():
                    async with self.current_channel.typing():
                        ...
                        try:
                            ...
                            logger.exception(f"Błąd podczas przetwarzania komunikatu: {e}")
                        finally:
    ...
    async def enqueue_message(self, message, user_message):
        await message.response.defer(ephemeral=self.isprywatny) if self.is_replying_all == "False" else None
    ...
    async def send_message(self, message, user_message):
        ...
        try:
            ...
            logger.exception(f"Błąd podczas wysyłania : {e}")
    ...
    async def send_start_prompt(self):
        ...
        try:
            if self.starting_prompt and discord_channel_id:
                ...
                logger.info(f"Wyślij monit systemowy z rozmiarem {len(self.starting_prompt)}")

                response = await self.handle_response(self.starting_prompt)
                await channel.send(f"{response}")

                logger.info(f"Szybka odpowiedź systemu: {response}")
            else:
                logger.info("Nie podano monitu początkowego lub nie wybrano kanału Discord. Pomijanie wysyłania monitu systemu.")
        except Exception as e:
            logger.exception(f"Błąd podczas wysyłania monitu systemowego: {e}")
